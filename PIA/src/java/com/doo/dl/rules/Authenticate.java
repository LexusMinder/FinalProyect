...
public class Authenticate {
       ...
       public static boolean isValid(String username, String password, int autorizacion) {
            LoginDAO dao = new LoginDAO();
            User user1 = dao.obtenerUsername(username);
            String userDAO = user1.getUsername();
            String passwordDAO = user1.getPassword();
            int autorizacionDAO = user1.getAutorizacion();
            
            if (username.equals(userDAO) && password.equals(passwordDAO) && autorizacion == autorizacionDAO ) {
                return true;
            } else {
                return false;
            }
